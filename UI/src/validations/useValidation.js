...
export const useValidation = (validator) => {
  ...
  const onChangeInput = (ev) => {
    ...
    setErrors({
      ...errors,
      [name]: formData[name].validations.find((validation) => !validation.check(inputValue)).errorMessage,
    });
  };
  ...
  const handleCheckFormErrors = () => {
    ...
    Object.keys(values).map((key) => {
      const validation = formData[key].validations.find((validation) => !validation.check(values[key]));
