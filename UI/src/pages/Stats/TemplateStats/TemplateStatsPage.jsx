...
import TemplateStatsItem from "./TemplateStatsItem";

const TAKE_CONSTANT = 10;

function TemplateStatsPage() {
  let { userId } = useParams();

  const { getTemplatesInfo } = useTemplateService();
  ...
  useEffect(() => {
    async function f() {
      setIds(await getTemplatesInfo(skip, userId, input.current.value ?? ""));
    }
    f();
  }, []);
  ...
  async function fetchData() {
    await fetchDataForScroll(
      setIds,
      skip,
      async () => await getTemplatesInfo(TAKE_CONSTANT + skip, userId, input.current.value.trim())
    );
  ...
  function update() {
    ...
    timeout = setTimeout(async () => {
      setIds(await getTemplatesInfo(TAKE_CONSTANT, userId, input.current.value.trim()));
    }, 1000);
  }
  return (
    <div className="col col-9">
      ...
      <InfiniteScroll dataLength={ids.length} next={fetchData} loader={<h4>Loading...</h4>} hasMore={true}>
        <div className="row">
          {ids.map((id) => (
            <TemplateStatsItem key={id.templateId} templateInfo={id} userId={userId} />
          ))}
        </div>
      </InfiniteScroll>
    </div>
...
export default TemplateStatsPage;
